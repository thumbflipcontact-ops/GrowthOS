...
from app.core.entitlements import is_org_entitled, require_org_entitled
...
async def _make_org(db_session, *, suffix: str | None = None) -> Organization:
    ...
    return await OrganizationRepository(db_session).add(
        Organization(name="Acme", slug=f"acme-entitle-{suffix}")
    )
...
@pytest.mark.asyncio
async def test_org_with_no_subscription_is_not_entitled(db_session) -> None:
...
@pytest.mark.asyncio
async def test_require_org_entitled_raises_402_for_unentitled_org(db_session) -> None:
    org = await _make_org(db_session)
...
async def _register_and_create_project(api_client: AsyncClient, db_session, *, suffix: str) -> str:
...
@pytest.mark.asyncio
async def test_create_plugin_connection_is_blocked_without_a_subscription(
...
@pytest.mark.asyncio
async def test_trigger_agent_run_is_blocked_without_a_subscription(
    api_client: AsyncClient, db_session
) -> None:
...
@pytest.mark.asyncio
async def test_billing_status_for_org_with_no_subscription(
    api_client: AsyncClient, db_session
) -> None:
