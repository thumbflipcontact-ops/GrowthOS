...
See `docs/plugins/QUICKSTART.md` for the full runnable walkthrough. Short version:

1. `python scripts/new_plugin.py <name>` scaffolds `plugins/<name>/` — `manifest.py`,
   `plugin.py`, `README.md`, `pyproject.toml` (entry point + packaging metadata already
   correct), `tests/test_contract.py`. There is no required `client.py` — if your plugin
   benefits from separating its raw HTTP client from the Protocol-implementing `plugin.py`,
   that's a fine convention to adopt, but it's your call, not a required file.
...
4. The entry point in `pyproject.toml` (`[project.entry-points."growthos.plugins"]`) is
   already generated by step 1 — **this is the only file outside the plugin's own package
   that ever needs an edit**, and it's plugin-package metadata, not core code.
5. Write tests, including the shared contract test suite
   (`plugins/_shared/tests/test_plugin_contract.py`, already wired up by `tests/test_contract.py`
   from step 1) run against your plugin — this is what stops "add a plugin" from becoming
   "add a plugin and also debug seven call sites."
